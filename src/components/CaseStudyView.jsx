...
const GallerySubGrid = ({ images }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col justify-between p-2">
      {/* Big Preview Box */}
      <div className="flex-1 flex items-center justify-center min-h-[180px] lg:min-h-[260px] border border-[var(--color-primary)]/10 bg-[var(--color-panel)] p-4 relative overflow-hidden">
      ...
      <div className="flex gap-2 mt-4 overflow-x-auto justify-start lg:justify-center py-2 select-none" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        ...
        {images.map((img, idx) => (
          <div 
            ...
            className={`w-12 h-12 border cursor-pointer flex-shrink-0 p-1 bg-white transition-all duration-200 ${
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12 overflow-hidden">
        <AnimatePresence mode="wait">
          <motion.div 
            ...
            className="w-full h-full max-w-5xl grid grid-cols-1 lg:grid-cols-12 gap-8 items-center"
          >
            {/* Left Box: Graphic Canvas */}
            <div className="lg:col-span-6 h-[260px] lg:h-[400px] border border-[var(--color-primary)]/10 bg-[var(--color-panel)] relative flex items-center justify-center p-6 shadow-xs">
              ...
              {activeSlide.showMeta ? (
                <>
                  {activeSlide.subtitle && (
                    <div className="mb-3">
                      <span className="inline-block bg-[var(--color-secondary)] text-[var(--color-primary)] font-mono text-[10px] font-bold px-2 py-1 tracking-wider uppercase">
                        {activeSlide.subtitle}
                      </span>
                    </div>
                  )}
                  <h2 className="text-3xl lg:text-4xl font-display font-bold leading-none uppercase tracking-tighter text-[var(--color-primary)] mb-4">
                    {activeSlide.title}
                  </h2>
                </>
              ) : (
                <div className="mb-4">
                  ...
                  <h3 className="text-2xl lg:text-3xl font-display font-bold leading-none uppercase tracking-tighter text-[var(--color-primary)]">
                    {activeSlide.title}
                  </h3>
                </div>
              )}

              <div className="w-12 h-0.5 bg-[var(--color-theme)] mb-4" />

              {/* Narrative Content */}
              <p className="font-sans text-base lg:text-lg text-[var(--color-primary)]/80 leading-relaxed">
                {activeSlide.content}
              </p>

              {/* Meta details if overview */}
              {activeSlide.showMeta && (
                <div className="mt-6 flex flex-wrap gap-4 border-t border-[var(--color-primary)]/10 pt-4 font-mono text-[10px] text-[var(--color-primary)]/40">
                  {legend.client && (
                    <div>
                      <span className="uppercase block font-bold">Client:</span>
                      <span className="text-[var(--color-primary)]/70">{legend.client}</span>
                    </div>
                  )}
                  {legend.year && (
                    <div>
                      <span className="uppercase block font-bold">Year:</span>
                      <span className="text-[var(--color-primary)]/70">{legend.year}</span>
                    </div>
                  )}
                  {legend.location && (
                    <div>
                      <span className="uppercase block font-bold">Location:</span>
                      <span className="text-[var(--color-secondary)] font-bold">{legend.location.name}</span>
                    </div>
                  )}
                </div>
              )}
